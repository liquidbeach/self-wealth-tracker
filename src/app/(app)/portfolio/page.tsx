...
async function fetchLivePrice(symbol: string): Promise<{ price: number; change: number; changePercent: number } | null> {
  try {
    ...
    return { price: data.price || 0, change: data.change || 0, changePercent: data.changePercent || 0 }
  } catch {
    return null
  }
}
...
export default function PortfolioPage() {
    ...
    const updatedHoldings = await Promise.all(
      holdingsToUpdate.map(async (holding) => {
        ...
        if (holding.current_price && holding.current_price > 0) {
          return { ...holding, live_price: undefined, live_change: undefined, live_change_percent: undefined, price_error: false }
        }
        ...
        return { ...holding, live_price: liveData.price, live_change: liveData.change, live_change_percent: liveData.changePercent, price_error: false }
      })
    )
  ...
  const toggleExpanded = (holdingId: string) => {
    ...
    if (newExpanded.has(holdingId)) newExpanded.delete(holdingId)
    else newExpanded.add(holdingId)
  ...
  const getCashByCurrency = (currency: string): number => {
    return cashBalances.filter(c => c.currency === currency).reduce((sum, c) => sum + Number(c.balance), 0)
  }
  ...
  const formatCurrency = (value: number, currency: string) => `${getCurrencySymbol(currency)}${value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
  const formatCompact = (value: number) => Math.abs(value) >= 1000 ? `$${(value / 1000).toFixed(1)}k` : `$${value.toFixed(0)}`
                  ...
                  {currencyHoldings.map((holding) => {
                    ...
                    return (
                      <div key={holding.id} className="p-3">
                        <div className="flex items-center justify-between cursor-pointer" onClick={() => lots.length > 0 && toggleExpanded(holding.id)}>
                          <div className="flex items-center gap-2">
